...
<div class="btn-toolbar list-toolbar">
    <button class="btn btn-primary"><a href="<?php echo U('uadd');?>"><i class="fa fa-plus"></i>添加用户</a></button>
...
<table class="table">
  ...
  <tbody>
    <?php if(is_array($ulist)): $k = 0; $__LIST__ = array_slice($ulist,0,10,true);if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
      <td><?php echo ($k); ?></td>
      <td>
      <?php if($vo["name"] != null): echo ($vo["name"]); ?>
      <?php else: ?>
      <?php echo ($vo["phonenum"]); endif; ?>
      </td>
      <td><?php echo ($vo["phonenum"]); ?></td>
      <td>
      	<?php if(is_array($teams)): $i = 0; $__LIST__ = $teams;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$team): $mod = ($i % 2 );++$i; if(($vo['carteam'] == $team['id'])): echo ($team['name']); endif; endforeach; endif; else: echo "暂时没有数据" ;endif; ?>
      </td>
      <!-- <td><?php echo ($vo["cardid"]); ?></td>
      <td><?php echo ($vo["cardfontpic"]); ?></td>
      <td><?php echo ($vo["drivercard"]); ?></td>
      <td><?php echo ($vo["driverlicense"]); ?></td> -->
      <td>
      	  <?php if(($vo['type'] == 0)): ?>乘客<?php endif; ?> 
      	  <?php if(($vo['type'] == 1)): ?>顺风车<?php endif; ?> 
		  <?php if(($vo['type'] == 2)): ?>专车司机<?php endif; ?> 
		  <?php if(($vo['type'] == 3)): ?>专线车司机<?php endif; ?> 
		  <?php if(($vo['type'] == 4)): ?>调度<?php endif; ?>
      </td>
      <td>
      	  <?php if(($vo['iscert'] == 0)): ?>未认证<?php endif; ?> 
      	  <?php if(($vo['iscert'] == 1)): ?>司机认证中<?php endif; ?> 
		  <?php if(($vo['iscert'] == 2)): ?>司机认证成功<?php endif; ?> 
		  <?php if(($vo['iscert'] == 3)): ?>用户认证成功<?php endif; ?> 
		  <?php if(($vo['iscert'] == 4)): ?>用户认证中<?php endif; ?>
      </td>
      <td>
      	  <?php if(($vo['able'] == 0)): ?>禁用<?php endif; ?> 
		  <?php if(($vo['able'] == 1)): ?>可用<?php endif; ?>
      </td>
      <td>
          <a href="<?php echo U('uedit',array('uid'=>$vo['id']));?>"><i class="fa fa-pencil"></i></a>
          <a href="<?php echo U('udetail',array('uid'=>$vo['id']));?>"><i class="glyphicon glyphicon-zoom-in"></i></a>
          <a href="<?php echo U('ucert',array('uid'=>$vo['id'],'usertype'=>$vo['type']));?>"  onclick="return cert()" role="button" ><i class="glyphicon glyphicon-copyright-mark"></i></a>
          <a href="<?php echo U('udelete',array('uid'=>$vo['id']));?>" onclick="return del()" role="button" data-toggle="modal"><i class="fa fa-trash-o"></i></a>
      </td>
